...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */)           \
  ...
  INSTPAT("??????? ????? ????? 110 ????? 11000 11", bltu, B, src1 < src2 ? s->dnpc = s->dnpc - 4 + imm : 1);
  ...
  INSTPAT("??????? ????? ????? 000 ????? 00000 11", lb, I, word_t a = Mr(src1 + imm, 1); R(dest) = (a & 0x80) ? (a | 0xffffff00) : a);
  ...
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh, R, long long ans = (long long)src1 * (long long)src2; R(dest) = ans >> 32);
