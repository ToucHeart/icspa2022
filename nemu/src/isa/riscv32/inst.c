...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */)           \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq, B, src1 == src2 ? s->dnpc = s->dnpc - 4 + imm : 0);
  INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne, B, src1 != src2 ? s->dnpc = s->dnpc - 4 + imm : 0);
  INSTPAT("0000001 ????? ????? 000 ????? 01100 11", mul, R, R(dest) = src1 * src2);
  // INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge, B, src1 >= src2 ? s->dnpc = s->dnpc - 4 + imm : 0);
  // INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt, B, src1 < src2 ? s->dnpc = s->dnpc - 4 + imm : 0);
